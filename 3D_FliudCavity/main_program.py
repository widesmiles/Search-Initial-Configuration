...
main_folder = r"F:\LYF\FC-InitialConfiguration\Debug"
...
Pend = FC_info(os.path.join(main_folder,initial_step1_name))
initial_NodesInfo = ObtainNodeInfo(os.path.join(main_folder,'Step-1.inp'))
initial_Nodes = np.array(initial_NodesInfo)
initial_Nodes[:,0] = initial_Nodes[:,0] + 1         # convert to GLOBAL NODE number
initial_X0 = EffectiveNodes(initial_Nodes, dis_data[:,0])
X0 = np.array(initial_X0)[:,1:]
Ptarget = 1
Pstart = Ptarget
Pstart = Pstart - 20*(Pend - Ptarget)
with open('P_FC.txt','w') as P_FC:
    P_FC.writelines(str(Pstart))
...
while  loop_time<1 or disp_magnitude(Xn, X0).max()>absolute_tol or abs(Pend-Ptarget)>absolute_tol:
    ...
    subprocess.Popen('abaqus cae nogui={}'.format('GenerateStep3.py'),shell=True)
    step3_code = 0
    ...
    print(f'Pressure error is {Pend-Ptarget}')