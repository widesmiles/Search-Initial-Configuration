...
def dis_info(dat_name):
    ...
    with open(dat_name) as initial_step1:
        ...
        for line in lines:
            ...
            if '   THE FOLLOWING TABLE IS PRINTED FOR NODES BELONGING TO NODE SET ASSEMBLY_FC_AQUEOUS' in line:
...
def AddNodePrint(inpname):
    ...
    new_str = new_str + 'U\n'
    new_str = new_str + '*node print, nset=FC_aqueous\n'
    new_str = new_str + 'PCAV\n'
    new_str = new_str + '*node print, nset=FC_vitreous\n'
    new_str = new_str + 'PCAV\n'
...
def ObtainNodeInfo(initial_inp_filename):
    ...
    with open(initial_inp_filename) as initial_inp:
        ...
        end_count = 1
        for line in lines:
            if '*Part, name=RightEye_axi' in line and start_count==1 :
...
def FC_info(dat_name):
    ...
    for i,line in enumerate(lines):
        if '   THE FOLLOWING TABLE IS PRINTED FOR NODES BELONGING TO NODE SET ASSEMBLY_FC_VITREOUS' in line:
            P_line = lines[i-10]
            break
    P_end1 = float(P_line.split()[-1])
    
    for i,line in enumerate(lines):
        if '          THE ANALYSIS HAS BEEN COMPLETED' in line:
            P_line = lines[i-9]
            break
    P_end2 = float(P_line.split()[-1])
